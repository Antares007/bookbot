...
type hasher_t = {|
  update: (Buffer | string) => hasher_t,
  digest: () => Buffer,
|};
export function create(): hasher_t {
  var h0 = 0x67452301;
  var h1 = 0xefcdab89;
  var h2 = 0x98badcfe;
  var h3 = 0x10325476;
  ...
  var shift = 24;
  var totalLength = 0;
  const hasher = { update: update, digest: digest };
  return hasher;
  ...
  function update(chunk) {
    ...
    for (var i = 0; i < length; i++) write(chunk[i]);
    return hasher;
  }
  ...
  function updateString(string) {
    ...
    totalLength += length * 8;
    for (var i = 0; i < length; i++) write(string.charCodeAt(i));
    return hasher;
  }
  ...
  function digest() {
    ...
    for (var s = 24; s >= 0; s -= 8) write(totalLength >> s);

    // At this point one last processBlock() should trigger and we can pull out the result.
    //return toHex(h0) + toHex(h1) + toHex(h2) + toHex(h3) + toHex(h4);
    const hash = Buffer.allocUnsafe(20);
    hash.writeInt32BE(h0, 0);
    hash.writeInt32BE(h1, 4);
    hash.writeInt32BE(h2, 8);
    hash.writeInt32BE(h3, 12);
    hash.writeInt32BE(h4, 16);
    return hash;
  }
